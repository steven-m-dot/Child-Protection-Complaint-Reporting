...
/* ---------------------------------------------------------------------- */
/*  Claude API helper (structured extraction only — reports stay grounded  */
/*  in what was actually typed, never fabricated)                          */
/* ---------------------------------------------------------------------- */
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const stored = await window.storage.get("gsheet_sync_url");
  ...
  const saveSyncUrl = async (url) => {
    setSyncUrl(url || null);
    try { await window.storage.set("gsheet_sync_url", url); } catch (e) {}
